import { Auth0Plugin, Database, HostHelpers, PassportCallback, User } from "library";
...
(function (module: NodeModule) {
  ...
  var Auth0: Auth0Plugin = {
    async getStrategy(strategies, callback) {
      try {
        ...
        if (settings.id && settings.secret) {
          ...
          }, <PassportCallback>function (req, token, unused, unused2, profile, done) {
    ...
    async login(auth0Id, username, email, picture, callback) {
      ...
      try {
        ...
        if (uid) callback(null, { uid });
        else {
          // New User
          async function postRegistration(uid: string) {
          ...
          if (!uid) {
            ...
            postRegistration(uid);
          } else {
            postRegistration(uid); // Existing account -- merge
          }
        }
      } catch (err) {
        callback(err);
